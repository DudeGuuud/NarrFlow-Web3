module narr_flow::token {
    ...
    // NARR 代币类型
    struct NARR has drop {}
    
    // 平台财务管理结构
    struct Treasury has key {
        id: UID,
        balance: Balance<NARR>,
        admin: address
    }
    
    // === 事件 ===
    
    struct TokensRewarded has copy, drop {
        receiver: address,
        amount: u64,
        reward_type: u8,
        story_id: Option<ID>
    ...
    fun init(ctx: &mut TxContext) {
        ...
        // 创建NARR代币
        let (treasury_cap, metadata) = coin::create_currency<NARR>(
            NARR {},
            9, // 小数位
            b"NARR", // 符号
            b"NarrFlow Token", // 名称
            b"Collaborative storytelling platform token", // 描述
            option::none(), // 图标URL
            ctx
        );
